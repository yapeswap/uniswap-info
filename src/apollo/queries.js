...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "yapeswap/yapeswap-t1") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const V1_DATA_QUERY = gql`
  query yapeswap($date: Int!, $date2: Int!) {
    ...
    oneDay: yapeswapHistoricalDatas(where: { timestamp_lt: $date }, first: 1, orderBy: timestamp, orderDirection: desc) {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
...
export const GLOBAL_CHART = gql`
  query yapeswapDayDatas($startTime: Int!, $skip: Int!) {
    yapeswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
